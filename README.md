...
It starts and operates a WorkerPool to handle many tasks in parallel.
Client needs to call /api/pbkdf2/create to create a task and gets back the ID of the task.
Then the client polls the service to check the status and finally gets a result.

This Nest.js service has 2 controllers:
1. API to listen for requests
2. Processor - endless loop to continuously look for tasks and forward them to a Worker.

In prod environment, these services start as separate PM2 processes and work independently. 